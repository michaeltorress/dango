...
pub(super) fn create_order(
    ...
    let pair = PAIRS
        .may_load(storage, (&order.base_denom, &order.quote_denom))? // @audit no check base < quote?
        .ok_or_else(|| {
            anyhow!(
    ...
    // amount necessary for creating this order. // @audit no check direction?
    ...
    let remaining = amount.checked_into_dec()?; // @audit ?
    ...
    if order.time_in_force == TimeInForce::GoodTilCanceled {
        increase_liquidity_depths(
            ...
            remaining,
            &pair.bucket_sizes,
        )?; // @audit if pair is updated, decreasing reverts
    }
