...
pub fn add_initial_liquidity(deposit: &CoinPair) -> MathResult<Uint128> {
    normalized_invariant(deposit)?.checked_mul(INITIAL_LP_TOKEN_MULTIPLIER) // @audit 
}
...
pub fn add_subsequent_liquidity(
    ...
    Ok(invariant_ratio.checked_sub(Price::ONE)?) // @audit ?
}
...
pub fn swap_exact_amount_in(
    ...
    // order reflection.
    output_amount_before_fee.checked_mul_dec_floor(Udec128::ONE - *fee_rate) // @audit precision?
}
...
pub fn reflect_curve(
    ...
    let marginal_price = Price::checked_from_ratio(quote_reserve, base_reserve)?; // @audit diff with geometric?
