...
pub fn fill_orders(
    ...
    outcome.extend(fill_asks(
        asks,
        clearing_price,
        volume, // @audit changed in the fill_bids?
        current_block_height,
        maker_fee_rate,
        taker_fee_rate,
    )?);
...
fn fill_bids(
    ...
    for (order_price, mut order) in bids {
        ...
        order.fill(filled_base)?; // @audit no check slippage?
        volume -= filled_base;
        ...
        let refund_quote = filled_base.checked_mul(order_price - clearing_price)?; // @audit no consider the refund_base?
...
fn fill_asks(
    ...
    for (_, mut order) in asks {
        ...
        let refund_quote = filled_quote.checked_sub(fee_quote)?; // @audit ?
