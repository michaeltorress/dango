...
pub fn add_subsequent_liquidity(
    ...
    Ok(deposit_value.checked_div(reserve_value)?) // @audit ?
}
...
pub fn reflect_curve(
    ...
    let marginal_price = {
        const PRECISION: Uint128 = Uint128::new(1_000_000); // @audit ?
    ...
    let bids = {
        ...
        iter::from_fn(move || {
            ...
            let size_in_quote = remaining_quote.checked_mul_dec(*params.ratio).ok()?; // @audit swapping size_in_quote with several times can cause the unfair swap
    ...
    let asks = {
        let one_plus_fee_rate = Udec128::ONE.checked_add(*swap_fee_rate)?; // @audit ? inconsistency with bid
